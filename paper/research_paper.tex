...
Sensitivity at 90\% specificity (\satninety{}) on \vindr{}: \textbf{0.6566} (best ensemble) vs 0.1636 (\rsna{} --- large domain gap, indicating that the conservative end of the decision surface is most affected by domain shift).

\section{Introduction}

\lettrine[lines=2, lraise=0.1]{B}{reast} cancer is the most commonly diagnosed cancer worldwide and a leading cause of cancer-related death in women \cite{4}. Population screening programmes based on full-field digital mammography (FFDM) have repeatedly been shown to reduce breast-cancer mortality, yet they impose a heavy operational burden: in many countries each screening exam is double-read by two radiologists, and reader sensitivity varies by 10--30 percentage points between individuals \cite{10,11}. False-negative rates of 10--30\% and false-positive recall rates of approximately 8\% are well documented across both clinical practice and large computer-aided diagnosis (CAD) literature, with the historical, hand-engineered CAD systems failing to deliver measurable benefit in routine screening \cite{17}.
...
Dropout layers enabled at inference following Kendall and Gal \cite{13}; BatchNorm in eval mode. $N=20$ stochastic passes per test pair; mean probability $\bar{p}$ and binary entropy $H(\bar{p})=-\bar{p}\log\bar{p}-(1-\bar{p})\log(1-\bar{p})$ computed as uncertainty estimate. The classification threshold is selected by maximising Youden's~J index ($J = \text{sensitivity} + \text{specificity} - 1$) on the validation set.
...
All models trained on NVIDIA RTX 5070 (12\,GB VRAM) with bfloat16 and AdamW (weight decay 1e-4). Primary metric: AUROC. Also reported: F1 at Youden threshold, sensitivity, specificity, and \textbf{\satninety{}} (sensitivity at 90\% specificity). 95\% bias-corrected and accelerated bootstrap confidence intervals (BCa CIs, $n$=2000 resamples) on the 2,000-pair test split (99 cancer-positive). Typical AUROC CI width: $\pm$0.04--0.05.
...
\textbf{All four pairs show annotation consistently degrades AUROC by 0.122--0.201.} At epoch 4 the backbone unfreezes and fits the white box outlines as a shortcut texture; training F1 reaches $\sim$0.97 while test AUROC collapses because boxes are absent at test time. Potential remedies may include: (a) box-dropout regularisation or (b) test-time box injection via a lesion detector.
...
MC Dropout slightly improves AUROC (+0.003) and produces a meaningful sensitivity shift (+3.0 percentage points). More importantly, the mean entropy is \textbf{0.5670} on cancer-positive pairs versus \textbf{0.5112} on negatives --- a 0.056-nat gap that enables the Stage~6 triage policy.
...
Three factors may drive the gap: (1) Vietnamese vs United States breast density distribution and lesion prevalence; (2) different FFDM equipment and post-processing pipelines; (3) the S@90\%spec drop ($-75.1\%$) is particularly stark at the clinical operating point, suggesting the conservative side of the decision surface is most affected by domain shift. Table~\ref{tab:model-family-summary} summarises the best-performing model family at each stage.
...
\textbf{CrossView Transformer vs BreastPairNet.} ConvNeXt-Base (AUROC=0.8317) vs EfficientNet-B3 +reg (AUROC=0.8256): $\Delta$=0.006, which is within the $\pm$0.04 BCa CI---these models are not statistically separated. One plausible explanation is that exchanging spatial feature maps before global pooling may preserve localisation information that BreastPairNet's independent pooling discards, though confounded by the different backbone architectures.
...
\textbf{MC Dropout uncertainty.} The +0.003 AUROC gain from MC Dropout is within the 95\% BCa CI and should not be over-interpreted. The more clinically meaningful finding is the sensitivity boost (+3.0 percentage points, AUROC 0.8073$\to$0.8099) and the 0.056-nat entropy gap between cancer-positive and cancer-negative classes, which enables the Stage~6 triage protocol.
...
\textbf{Confidence intervals.} With only 99 cancer-positive test pairs, BCa CI width is $\pm$0.04--0.05 AUROC. Differences of less than 0.01 AUROC should be treated as within sampling noise. BreastPairNet (AUROC=0.8256, 95\% CI 0.7755--0.8707) and CrossView Transformer (AUROC=0.8317, 95\% CI 0.7832--0.8753) are not statistically separated.
