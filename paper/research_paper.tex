...
\title{\textbf{Multi-View Fusion, CrossView Transformer, and Uncertainty-Aware Triage for Breast Cancer Detection on \vindr}\\[0.4em]
\large From 0.756 to 0.846 AUC}
...
\textbf{Background:} Breast cancer is the most commonly diagnosed cancer worldwide. Population screening based on full-field digital mammography (FFDM) reduces mortality but imposes heavy radiologist workload and substantial inter-reader variability, motivating computer-aided diagnosis (CAD).

\smallskip
\textbf{Purpose:} To develop and evaluate a six-stage pipeline for breast cancer detection on the \vindr{} FFDM benchmark, progressing from single-view CNN baselines to a multi-view ensemble with uncertainty-aware triage simulation.

\smallskip
\textbf{Materials and Methods:} Five single-view CNN architectures (Stage~1), BreastPairNet shared-backbone multi-view network across twelve variants (Stage~2), CrossView Transformer with spatial cross-attention (Stage~3), bounding-box annotation ablation (Stage~3b), MC Dropout uncertainty quantification (Stage~4), 16-model ensemble with TTA (Stage~5), and Verboom-style hybrid triage simulation (Stage~6). External validation on \rsna{}.

\smallskip
\textbf{Results:} DenseNet-121 attained AUC=0.7564 (Stage~1). BreastPairNet improved to AUC=0.8256 (+9.1\%, Stage~2); CrossView Transformer reached AUC=0.8317 (+10.0\%, Stage~3). Bounding-box annotations degraded AUC by 0.12--0.20. The 16-model ensemble achieved AUC=0.8458 (+11.8\%) with \satninety{}=0.6566. Hybrid triage at 50\% AI reads preserved 87.9\% sensitivity. External \rsna{} AUC=0.6008.

\smallskip
\textbf{Conclusion:} A six-stage multi-view pipeline achieves AUC=0.8458 on \vindr{}. Bounding-box shortcut learning is a key negative finding; domain generalisation to US FFDM remains an open challenge.
\end{tcolorbox}
\smallskip
\noindent\textbf{Keywords:} breast cancer detection; mammography; multi-view learning; uncertainty quantification; triage; \vindr{}.
\vspace{1.2em}
}]
...
\begin{table*}[tbp]
\centering
\caption{Summary of the main AUC results across the six-stage pipeline.}
...
\textbf{Stage 3b:} annotation ablation, all painted-box variants lose 0.122--0.201 AUC.\\
...
\end{minipage}}
\caption{Overview of the six-stage breast cancer detection pipeline.}
\label{fig:pipeline}
\end{figure*}

\section{Related Work}

\textbf{Deep CNN backbones.} Residual networks \cite{7} and densely connected networks \cite{8} underpin almost all modern CAD systems. CheXNet \cite{6} established DenseNet-121 as a default starting point for chest imaging. EfficientNet \cite{9} introduced compound scaling (B3: 12M parameters). ConvNeXt \cite{16} modernised pure-convolutional design to transformer-level ImageNet accuracy.
...
\vindr{} \cite{1} contains 20,000 FFDM images from 5,000 patients, four views per study (R-CC, L-CC, R-MLO, L-MLO) stored as DICOM, with BI-RADS (1--5), density (A--D), and lesion-level bounding box annotations.
...
\subsection{DICOM preprocessing (12/14-bit \(\rightarrow\) 8-bit PNG)}

Four-step pipeline: (1) decode with pydicom+\texttt{pylibjpeg}; (2) percentile-clip to [1\%,\,99\%]; (3) CLAHE \cite{14} (32$\times$32 tiles, clip-limit 2.0); (4) standardise orientation via horizontal flip when left half is brighter. Output: single-channel 8-bit PNG replicated to 3 channels.

\subsection{Single-view CNN backbones}

Five configurations: \textbf{VGG19}, \textbf{ResNet-50} (224\,px), \textbf{ResNet-50} (384\,px), \textbf{EfficientNet-B3} (224\,px), \textbf{DenseNet-121} (224\,px). All use ImageNet weights, focal loss ($\alpha$=0.25, $\gamma$=2.0) \cite{5}, WeightedRandomSampler, and Adam with cosine decay. Training: 20 epochs with early stopping on validation AUC.

\subsection{BreastPairNet (Stage 2)}

Shared backbone produces per-view global-average-pooled vectors $\mathbf{f}_{CC},\mathbf{f}_{MLO}\in\mathbb{R}^d$. Two fusion heads: \textbf{concatenation} (2d-dim MLP, dropout 0.5) and \textbf{attention} (learned per-view scalar weights via softmax). The \textbf{+reg} variant adds stronger elastic deformation (16\,px), mixup ($\alpha$=0.4), and label smoothing (0.05). Training: backbone frozen 3 epochs, then discriminative LR (1e-5 backbone, 1e-4 head). \autoref{fig:breastpairnet-arch} illustrates the architecture.
...
% Feature vectors
\node[featb] (fcc)  at (7.2,  1.0) {$\mathbf{f}_{CC}\!\in\!\mathbb{R}^d$};
\node[featb] (fmlo) at (7.2, -1.0) {$\mathbf{f}_{MLO}\!\in\!\mathbb{R}^d$};

% Fusion head
\node[fusb] (fus) at (10.2, 0) {Fusion\\head\\(concat\\or attn)};

% MLP
\node[mlpb] (mlp) at (12.8, 0) {MLP\\+Drop(0.5)};

% Output
\node[outb] (out) at (15.0, 0) {$P$(cancer)};

% Arrows: inputs → backbone
\draw[arr] (cc.east)  to[out=0, in=150] (bb.north west);
\draw[arr] (mlo.east) to[out=0, in=210] (bb.south west);

% Arrows: backbone → features (split out east, go up/down)
\draw[arr] (bb.east) -- +(0.8, 0) |- (fcc.west);
\draw[arr] (bb.east) -- +(0.8, 0) |- (fmlo.west);
...
% GAP label
\node[font=\tiny, gray, align=center] at (5.5,  1.45) {global avg\\pool};
\node[font=\tiny, gray, align=center] at (5.5, -1.45) {global avg\\pool};
\end{tikzpicture}
\caption{BreastPairNet architecture (Stage~2). CC and MLO images from the same breast pass through a \emph{shared} CNN backbone (weights $\theta$ are tied for both views, indicated by the dashed arrow). Each view produces a global-average-pooled feature vector; the two vectors are fused by either concatenation (dim $2d$) or learned attention weights, then classified by a two-layer MLP with 50\% dropout.}
...
Dropout enabled at inference; BatchNorm in eval mode. $N=20$ stochastic passes per test pair; mean probability $\bar{p}$ and binary entropy $H(\bar{p})=-\bar{p}\log\bar{p}-(1-\bar{p})\log(1-\bar{p})$ computed as uncertainty estimate. Threshold optimised via Youden's~J.

\subsection{Hybrid triage (Verboom 2025 framework)}

Cases sorted by ascending entropy; the K most-confident are routed to AI and the remainder to a radiologist (modelled as oracle or 85\% sensitivity). Overall sensitivity = (TP$_\text{AI}$ + cancers referred) / total cancers.

\subsection{Training and evaluation}

All models trained on NVIDIA RTX 5070 (12\,GB VRAM) with bfloat16 and AdamW (weight decay 1e-4). Primary metric: AUROC. Also reported: F1 at Youden threshold, sensitivity, specificity, and \textbf{S@90\%} (sensitivity at 90\% specificity). 95\% bootstrap CIs (BCa, $n$=2000 resamples) on the 2,000-pair test split (99 cancer-positive). Typical AUC CI width: $\pm$0.04--0.05.

\section{Experiments and Results}

\subsection{Stage 1 --- Single-view CNN baselines}

DenseNet-121 achieves the best AUC (0.7564) but only 24.8\% sensitivity at the default threshold. All five models overfit (train F1~$\approx$~0.98) and reach only 40--45\% sensitivity at the 90\%-specificity operating point --- well below the $\sim$85\% radiologist sensitivity in screening literature \cite{17}.
...
\begin{axis}[
    ...
    ylabel={AUC}, ymin=0.68, ymax=0.78,
...
\end{axis}
\end{tikzpicture}
\caption{Stage~1: AUC by single-view architecture. ResNet-50* uses 384\,px; all others 224\,px.}
\label{fig:stage1-bar}
\end{figure*}

\subsection{Stage 2 --- Multi-view BreastPairNet}

The best single model is \textbf{EfficientNet-B3 + attention + +reg} at AUC=0.8256 (95\% CI 0.7755--0.8707), a +9.1\% relative gain over DenseNet-121 single-view.

\begin{table*}[tbp]
\centering
\caption{BreastPairNet ablation across backbone, fusion, and resolution.}
\label{tab:breastpairnet}
\begin{tabular}{@{}p{0.22\textwidth} p{0.09\textwidth} p{0.08\textwidth} p{0.20\textwidth} p{0.07\textwidth} p{0.07\textwidth} p{0.15\textwidth}@{}}
\toprule
...
DenseNet-121 \textbf{+reg} & attention & 224 px & 0.8097 [0.7594--0.8591] & 0.2033 & 0.7980 & 0.5556 [0.4301--0.6458] \\
DenseNet-121 & attention & 512 px & 0.8150 [0.7675--0.8617] & 0.1928 & 0.8081 & 0.5455 [0.4476--0.6408] \\
DenseNet-121 \textbf{+reg} & concat & 224 px & 0.8177 [0.7659--0.8649] & 0.2357 & 0.7677 & 0.6061 [0.5047--0.7000] \\
EfficientNet-B3 & attention & 512 px & 0.8175 [0.7698--0.8661] & 0.2807 & 0.7273 & 0.5556 [0.4495--0.6505] \\
\textbf{EfficientNet-B3 +reg} & \textbf{attention} & \textbf{224 px} & \textbf{0.8256 [0.7755--0.8707]} & \textbf{0.3701} & \textbf{0.6263} & \textbf{0.6263 [0.5229--0.7143]} \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{axis}[
    ...
    ylabel={AUC}, ymin=0.79, ymax=0.835,
...
\end{axis}
\end{tikzpicture}
\caption{Stage~2: Effect of +reg on AUC for three backbone/fusion pairs.}
\label{fig:stage2-reg-bar}
\end{figure*}

All point estimates in \autoref{tab:breastpairnet} and \autoref{tab:crossview} are obtained by re-evaluating each best-checkpoint model on the held-out test split (n=2,000 pairs, 99 positives) at the Youden-optimal threshold, with 95\% BCa bootstrap confidence intervals ($n$=2,000 resamples).

Three observations stand out. \textbf{(i) +reg consistently lifts AUC} by +0.008--0.021 across all backbone/fusion pairs. \textbf{(ii) Resolution scaling is not uniformly beneficial}: DenseNet-121 benefits from 512\,px (+0.014), but EfficientNet-B3 +reg at 224\,px (0.8256) beats 512\,px without +reg (0.8175). \textbf{(iii) ResNet-50 degrades at 384\,px} (0.7467 vs 0.7962 at 224\,px) due to batch-size reduction under VRAM pressure.
...
\begin{axis}[
    ...
    xlabel={Training epoch}, ylabel={Validation AUC},
...
\legend{Stage 2: EffB3 +reg (AUC=0.8256 at ep.~7), Stage 3: ConvNeXt-Base (AUC=0.8317 at ep.~8)}
\end{axis}
\end{tikzpicture}
\caption{Validation AUC vs.\ training epoch for the best Stage~2 (EffB3 +reg) and Stage~3 (ConvNeXt-Base) models. Vertical dashed lines mark the backbone unfreeze point (epoch~4 for EffB3, epoch~5 for ConvNeXt). Stars ($\star$) indicate the best-checkpoint epoch used for test-set evaluation. Both models overfit after their best epoch as training loss continues to decrease while validation AUC plateaus.}
...
\label{tab:crossview}
\begin{tabular}{@{}p{0.22\textwidth} p{0.13\textwidth} p{0.20\textwidth} p{0.07\textwidth} p{0.07\textwidth} p{0.18\textwidth}@{}}
\toprule
...
\textbf{ConvNeXt-Base} & \textbf{VinDr} & \textbf{0.8317 [0.7832--0.8753]} & \textbf{0.2587} & \textbf{0.7475} & \textbf{0.5758 [0.4737--0.6768]} \\
\bottomrule
\end{tabular}
\end{table*}

The best is \textbf{ConvNeXt-Base at AUC=0.8317} (+10.0\% over single-view). DenseNet-121 with \vindr{}+\rsna{} joint training reaches 0.8028, confirming incremental cross-dataset benefit. Medical pretraining (torchxrayvision) underperforms ImageNet (0.7648 vs 0.7874), suggesting domain mismatch.
...
\toprule
Architecture & No annotation AUC [95\% CI] & With annotation AUC [95\% CI] & \(\Delta\)AUC \\
\midrule
...
\begin{axis}[
    ...
    ylabel={AUC}, ymin=0.55, ymax=0.86,
    ytick={0.55,0.60,0.65,0.70,0.75,0.80,0.85},
    xtick=data,
...
\end{axis}
\end{tikzpicture}
\caption{Stage~3b: Annotation consistently degrades AUC due to shortcut learning and test-time distribution shift.}
\label{fig:annotation-bar}
\end{figure*}

\textbf{All four pairs show annotation hurts AUC by 0.122--0.201.} At epoch 4 the backbone unfreezes and fits the white box outlines as a shortcut texture; training F1 reaches $\sim$0.97 while test AUC collapses because boxes are absent at test time. Remedies: (a) box-dropout regularisation or (b) test-time box injection via a lesion detector.
...
\toprule
Mode & AUC & Sens & Spec & S@90\% & Thr. \\
\midrule
Deterministic & 0.8073 & 0.7374 & 0.7249 & 0.5152 & 0.3304 \\
\textbf{MC Drop.} & \textbf{0.8099} & \textbf{0.7677} & \textbf{0.7007} & \textbf{0.5253} & \textbf{0.3138} \\
\bottomrule
\end{tabular}}
\end{table}

MC Dropout slightly improves AUC (+0.003) and produces a meaningful sensitivity shift (+3.0 points). More importantly, the mean entropy is \textbf{0.5670} on cancer-positive pairs versus \textbf{0.5112} on negatives --- a 0.056-nat gap that enables the Stage~6 triage policy.
...
Mean-probability ensemble of 16 BreastPairNet checkpoints combined with three breast-asymmetry models and TTA$\times$5. \textbf{Best result in the study}: AUC=0.8458, sensitivity=0.7374, specificity=0.8296, F1=0.2944, S@90\%=0.6566, S@95\%=0.5657 at threshold 0.3414 (TP=73, TN=1577, FP=324, FN=26).

\begin{table*}[tbp]
\centering
\caption{Best ensemble result (mean probability, TTA$\times$5).}
\label{tab:ensemble}
\begin{tabular}{@{}p{0.20\textwidth} ccccccc@{}}
\toprule
Method & AUC & Sens & Spec & F1 & S@90\% & S@95\% & Threshold \\
\midrule
\textbf{Ensemble (N=16, TTA=5)} & \textbf{0.8458} & \textbf{0.7374} & \textbf{0.8296} & \textbf{0.2944} & \textbf{0.6566} & \textbf{0.5657} & \textbf{0.3414} \\
\bottomrule
\end{tabular}
\end{table*}
...
This ensemble does \textbf{not} include ConvNeXt-Base or EfficientNet-B3 +reg models.

\subsubsection{Updated ensemble (25 checkpoints)}

Adding all CrossView checkpoints (including lower-epoch snapshots) gives AUC=0.8441, S@90\%=0.6566 (unchanged), sensitivity=0.7677. The marginal AUC drop confirms that weaker models drag the mean under mean combination; S@90\% is robust.

\subsubsection{Selective ensemble (top-3 +reg + top-5 CrossView + 3 asymmetry)}

11 models, TTA$\times$5: AUC=0.8446, S@90\%=0.6667 (+0.010 over baseline). Marginally better at the clinical operating point; difference is within sampling noise.
...
Under oracle assumptions, \textbf{K=50\% delivers 87.9\% overall sensitivity with 50\% workload reduction}. Under realistic 85\% radiologist sensitivity \cite{17}, the same K=50\% yields 77.3\% --- below the clinical floor. Achieving $\geq$80\% overall sensitivity under realistic conditions requires K$\leq$40\%.
...
\begin{table}[h!]
\centering
\caption{AUC progression on \vindr{} binary cancer detection.}
...
\(\ddagger\)MamT4 excludes BI-RADS 3; our pipeline retains it as negative. McKinney et al. \cite{11} (AUC=0.889) and Yoon et al. \cite{4} meta-analysis (pooled AUC=0.87) use different datasets and are not directly comparable.
...
\begin{axis}[
    ...
    ylabel={AUC}, ymin=0.72, ymax=0.87,
...
\end{axis}
\end{tikzpicture}
\caption{AUC progression across pipeline stages vs.\ MamT4 prior work.}
...
\begin{tabular}{@{}lcccc@{}}
\toprule
Dataset & AUC [95\% CI] & S@90\% & Sens & Spec \\
\midrule
...
Three factors drive the gap: (1) Vietnamese vs US breast density distribution and lesion prevalence; (2) different FFDM equipment and post-processing; (3) the S@90\%spec drop (-75.1\%) is particularly stark at the clinical operating point, indicating the conservative side of the decision surface is most affected by domain shift. Table~\ref{tab:model-family-summary} summarises the best-performing model family at each stage.
...
\textbf{Why multi-view helps.} The +9.1\% gain from BreastPairNet is consistent with the radiological reality that lesions visible in CC may be ambiguous in MLO and vice versa. Joint conditioning removes view-specific false positives.

\textbf{Why +reg helps.} With only $\sim$395 cancer-positive training pairs, stronger augmentation (elastic deformation, mixup, label smoothing) is a powerful implicit regulariser (+0.008--0.021 AUC across all tested variants).

\textbf{Why CrossView edges out BreastPairNet.} ConvNeXt-Base (0.8317) vs EfficientNet-B3 +reg (0.8256): $\Delta$=0.006, within the $\pm$0.04 CI. Exchanging spatial feature maps before global pooling preserves localisation information that BreastPairNet's independent pooling discards.

\textbf{Why medical pretraining hurt.} Chest-X-ray features (lung parenchyma, mediastinum, rib edges) are domain-mismatched to mammographic texture. ImageNet's broader texture vocabulary provides a better starting point (0.7874 vs 0.7648).

\textbf{Why annotation always hurt.} Painted boxes provide a strong shortcut perfectly correlated with the cancer label in training, but absent at test time. Remedies: box-dropout regularisation or test-time box injection.

\textbf{MC Dropout.} The +0.003 AUC gain is within the 95\% CI. The real value is the sensitivity boost (+3.0 points) and the 0.056-nat entropy gap between classes, which enables triage.

\textbf{Triage simulation.} Under realistic 85\% radiologist sensitivity, K=50\% yields only 77.3\% overall sensitivity --- 10.6 points below the oracle result. K$\leq$40\% is required to stay above the 80\% clinical floor in non-oracle conditions.

\textbf{Confidence intervals.} With 99 positive test pairs, AUC CI width is $\pm$0.04--0.05. Differences $<$0.01 AUC are within sampling noise. BreastPairNet (0.8256) and CrossView (0.8317) are not significantly separated.

\textbf{Limitations.} (i) Small positive test count (99 pairs) limits fine-grained comparison; (ii) large domain gap to US FFDM (AUC=0.6008); (iii) MamT4 comparison uses a slightly different BI-RADS exclusion; (iv) 7,054 non-CC/MLO RSNA images excluded from pair construction.
...
We have presented a six-stage pipeline for breast cancer detection on \vindr{}: \textbf{Stage 1} (DenseNet-121, AUC=0.7564) $\rightarrow$ \textbf{Stage 2} (BreastPairNet +reg, AUC=0.8256, +9.1\%) $\rightarrow$ \textbf{Stage 3} (CrossView Transformer, AUC=0.8317, +10.0\%) $\rightarrow$ \textbf{Stage 5} (16-model ensemble, AUC=0.8458, +11.8\%). Bounding-box annotations consistently degraded AUC by 0.12--0.20 due to shortcut learning --- the most actionable negative finding. MC Dropout produced a 0.056-nat entropy gap enabling hybrid triage: 50\% AI reads preserves 87.9\% sensitivity under oracle conditions, but only 77.3\% under realistic 85\% radiologist sensitivity, requiring K$\leq$40\% to stay above the clinical floor. External validation (RSNA AUC=0.6008) reveals that domain generalisation to US FFDM remains a critical open challenge.
