"""
Medical-pretrained AsymmetryNet experiment entry point.
Called by run_parallel_cnn.py --phase medical.

Backbone: DenseNet-121 pretrained on 7 chest X-ray datasets via TorchXRayVision
(NIH+PC+CheXpert+MIMIC+Google+OpenI+Kaggle), fine-tuned on VinDr-Mammo.

Medical imaging pretraining provides better feature initialisation than ImageNet
for mammography — radiological textures transfer across modalities (chest→breast).

Usage:
    python scripts/run_single_medical.py \
        --key med_densenet121_e30 --arch densenet121 \
        --epochs 30 --lr 3e-5 --batch 6 --img 224 \
...
def main():
    ...
    print(f"\n{'='*65}")
    print(f"  MEDICAL-PRETRAINED ASYMMETRY: {args.key}")
    ...
    print(f"  backbone=TorchXRayVision (densenet121-res224-all)")
    print(f"  freeze={args.freeze} epochs")
    print(f"{'='*65}\n")
